...
class ColorConverter:
    ...
    def parse_color(self, color_input: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def normalize_hex_color(self, hex_input: str) -> Optional[str]:
    ...
    def _parse_hex(self, hex_value: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def _validate_rgb_components(self, components: str, expected_count: int) -> bool:
        ...
        try:
            ...
            if expected_count == 4:
                if not self._is_valid_alpha_component(parts[3]):
                    return False
    ...
    def _parse_rgb(self, rgb_values: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def _parse_rgba(self, rgba_values: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def _parse_hsl(self, hsl_values: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def _parse_hsla(self, hsla_values: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def _parse_hsb(self, hsb_values: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def _parse_hsv(self, hsv_values: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def is_valid_hwb_color(self, color_str: str) -> bool:
        ...
        try:
            ...
            if not self._is_valid_percentage_component(parts[1]):
                return False
            if not self._is_valid_percentage_component(parts[2]):
                return False
    ...
    def _parse_hwb(self, hwb_values: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def _parse_cmyk(self, cmyk_values: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    def _parse_color_component(self, value: str, max_value: Union[int, float]) -> Optional[float]:
    ...
    def _parse_hue(self, value: str) -> Optional[float]:
    ...
    def to_hex(self, r: float, g: float, b: float, a: float = 1.0, include_alpha: bool = None) -> str:
    ...
    def to_hex_short(self, r: float, g: float, b: float, a: float = 1.0) -> Optional[str]:
    ...
    def to_rgba(self, r: float, g: float, b: float, a: float = 1.0, use_percentages: bool = False, alpha_as_percentage: bool = False) -> str:
    ...
    def to_hsla(self, r: float, g: float, b: float, a: float = 1.0, precision: int = 0, alpha_as_percentage: bool = True) -> str:
    ...
    def to_hsla_with_deg(self, r: float, g: float, b: float, a: float = 1.0, precision: int = 0, alpha_as_percentage: bool = True) -> str:
...
def create_color_converter_widget():
    ...
    def update_color_formats(color_input: str):
        ...
        if not color_input.strip():
            ...
            for line_edit in format_line_edits.values():
                line_edit.clear()
            color_preview.setStyleSheet("QLabel#colorPreview { background-color: #ffffff; border: 1px solid #ced4da; border-radius: 4px; }")
            return
        ...
        if rgba is None:
            ...
            color_preview.setStyleSheet("QLabel#colorPreview { background-color: #ffffff; border: 2px solid #dc3545; border-radius: 4px; }")
            return
    ...
    def update_code_presets(rgba_values=None):
        ...
        red_b = int(round(r * 255))
        green_b = int(round(g * 255))
        blue_b = int(round(b * 255))
        alpha_b = int(round(a * 255))
        alpha_p = int(round(a * 100))
        
        # HSL values
        import colorsys
        h, l, s = colorsys.rgb_to_hls(r, g, b)
        hue_d = int(round(h * 360))
